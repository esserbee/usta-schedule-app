...
def extract_location_from_all_start_times_cell(cell_text):
    ...
    if best:
        ...
        if best_lower.startswith('stc') or best_lower in ('stc', 'stc courts', 'stc court', 'stc ct'):
    ...
    # Alias normalisation for Pattern 6 results (e.g., "STC Courts")
    if m:
        name = m.group(1).strip()
        suffix = m.group(2).strip()
        if is_valid_location(name):
            full = f"{name} {suffix.title()}"
            if full.lower().startswith('stc'):
                return 'Sunnyvale Tennis Center'
            return full
